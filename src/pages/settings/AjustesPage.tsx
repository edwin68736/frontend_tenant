...
export default function AjustesPage() {
  ...
  const defaultTab = useMemo((): ErpSettingsMainTab => {
    if (navState?.erpSettingsTab === 'impresoras' && canPrinters) return 'impresoras'
    if (navState?.erpSettingsTab === 'empresa' && canCompany) return 'empresa'
    if (canCompany) return 'empresa'
    if (canPrinters) return 'impresoras'
    return 'empresa'
  }, [canCompany, canPrinters, navState?.erpSettingsTab])

  const [tab, setTab] = useState<ErpSettingsMainTab>(defaultTab)
  const activeTab: ErpSettingsMainTab =
    canCompany && canPrinters ? tab : canCompany ? 'empresa' : 'impresoras'
  const printersOnly = canPrinters && !canCompany
          ...
          <div className="min-w-0">
            ...
            <p className="mt-0.5 sm:mt-1 text-xs sm:text-sm text-gray-600 hidden sm:block">
              {printersOnly
                ...
                : 'Configuración de la empresa y del equipo local.'}
            </p>
          </div>
        </div>

        {canCompany && canPrinters && (
