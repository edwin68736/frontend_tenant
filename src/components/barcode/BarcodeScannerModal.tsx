...
import { CAMERA_SCANNER_Z } from '@/utils/uiLayers'
  ...
  useEffect(() => {
    ...
    const start = async () => {
      ...
      if (permission !== 'granted') {
        setStarting(false)
        setError(
          'Permiso de cámara denegado. Actívalo en Ajustes → Aplicaciones → Tukifac → Permisos → Cámara.',
        )
        return
      }
        ...
        <div className="relative w-full max-w-lg aspect-[4/3] rounded-2xl overflow-hidden bg-black shadow-2xl ring-1 ring-white/10">
          ...
          <div className="pointer-events-none absolute inset-0 z-10 flex items-center justify-center">
            <div className="relative w-[88%] max-w-[340px] h-[32%] max-h-[140px]">
              ...
              <span
                className="absolute left-2 right-2 h-0.5 bg-primary-500/80 shadow-[0_0_12px_rgba(34,197,94,0.8)] animate-[scanLine_2s_ease-in-out_infinite]"
                style={{ top: '15%' }}
              />
            </div>
          </div>
